...
function compile(masm, wsroot, config) {
    try {
        const cparams = cppdecls_emitter_1.CPPEmitter.emit(masm, config.entrypoint);
        ...
        const entrypoint = masm.invokeDecls.get(config.entrypoint);
        if (entrypoint.params.some((p) => !masm.subtypeOf(masm.typeMap.get(p.type), masm.typeMap.get("NSCore::APIValue")))) {
...
function verify(masm, config, genmodel) {
    try {
        ...
        const sparams = smtdecls_emitter_1.SMTEmitter.emit(masm, entrypoint, true);
