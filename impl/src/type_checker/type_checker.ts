...
class TypeChecker {
    ...
    private emitCheckedInlineConvertIfNeeded<T extends MIRArgument>(sinfo: SourceInfo, src: T, srctype: ValueType, trgttype: ResolvedType, guard: MIRGuard): T | MIRTempRegister {
        if(srctype.layout.isSameType(trgttype)) {
            return src;
        }
    ...
    private checkArgumentsEntityConstructor(sinfo: SourceInfo, oftype: ResolvedEntityAtomType, args: ExpandedArgument[], trgt: MIRTempRegister): ResolvedType {
            ...
            if (arg.name !== undefined) {
                ...
                expandInfo[1].forEach((pname) => {
                    ...
                    if(expandInfo[0].has(pname)) {
                        ...
                        this.m_emitter.emitLoadRecordPropertySetGuard(sinfo, arg.treg as MIRTempRegister, this.m_emitter.registerResolvedTypeReference((arg.argtype as ValueType).layout), this.m_emitter.registerResolvedTypeReference((arg.argtype as ValueType).flowtype), pname, !(arg.argtype as ValueType).flowtype.isUniqueRecordTargetType(), ptype, etreg, new MIRMaskGuard(fflag, fidx - optfirst));
            ...
            if (!arg.expando) {
                ...
                for (let ectr = 0; ectr < expandInfo[1]; ++ectr) {
                    ...
                    if(ectr < expandInfo[0]) {
                        ...
                        this.m_emitter.emitLoadTupleIndexSetGuard(sinfo, arg.treg as MIRTempRegister, this.m_emitter.registerResolvedTypeReference((arg.argtype as ValueType).layout), this.m_emitter.registerResolvedTypeReference((arg.argtype as ValueType).flowtype), ectr, !(arg.argtype as ValueType).flowtype.isUniqueTupleTargetType(), itype, etreg, new MIRMaskGuard(fflag, ii - optfirst));
        ...
        for (let i = 0; i < fields.length; ++i) {
            ...
            if(filledLocations[i].fflagchk) {
                cargs.push(this.emitCheckedInlineConvertIfNeeded(sinfo, filledLocations[i].trgt, filledLocations[i].vtype, fieldtype, new MIRMaskGuard(fflag, i - optfirst)));
            }
            else {
            ...
            if (sig.params[j].type instanceof ResolvedFunctionType) {
                ...
                if (sig.params[j].refKind === undefined || sig.params[j].refKind === "ref") {
                    ...
                    if (filledLocations[j].fflagchk) {
                        narg = this.emitCheckedInlineConvertIfNeeded(sinfo, narg, filledLocations[j].vtype as ValueType, sig.params[j].type as ResolvedType, new MIRMaskGuard(fflag, j - optfirst));
                    }
                    else {
    ...
    private checkCreateTypedString(env: TypeEnvironment, exp: LiteralTypedStringExpression, trgt: MIRTempRegister): TypeEnvironment {
        ...
        if(oftype.getUniqueCallTargetType().object.specialDecls.has(SpecialTypeCategory.ValidatorTypeDecl)) {
            ...
            if (this.m_doLiteralStringValidate) {
                ...
                const skey = this.m_emitter.registerStaticCall(ctype, sdecl as StaticFunctionDecl, "tryParse", sbinds, [], []);
    ...
    private checkTypedTypedNumericConstructor(env: TypeEnvironment, exp: LiteralTypedNumericConstructorExpression, trgt: MIRTempRegister): TypeEnvironment {
        ...
        const skey = this.m_emitter.registerStaticCall(ctype, consf as StaticFunctionDecl, "create", new Map<string, ResolvedType>(), [], []);
    ...
    private checkAccessStaticField(env: TypeEnvironment, exp: AccessStaticFieldExpression, trgt: MIRTempRegister): TypeEnvironment {
        ...
        if (cexp !== undefined) {
            ...
            const skey = this.m_emitter.registerPendingConstProcessing(this.m_emitter.registerResolvedTypeReference(rctype), cdecl.decl, cdecl.binds, rtype);
    ...
    private checkConstructorPrimaryWithFactory(env: TypeEnvironment, exp: ConstructorPrimaryWithFactoryExpression, trgt: MIRTempRegister): TypeEnvironment {
        ...
        const skey = this.m_emitter.registerStaticCall(ootype, fdecl as StaticFunctionDecl, exp.factoryName, callbinds, rargs.pcodes, rargs.cinfo);
    ...
    private checkSpecialConstructorExpression(env: TypeEnvironment, exp: SpecialConstructorExpression, trgt: MIRTempRegister, infertype: ResolvedType | undefined): TypeEnvironment {
        ...
        if (exp.rop === "ok") {
            ...
            const okconskey = this.m_emitter.registerStaticCall(ootype, okconsfunc, "create", new Map<string, ResolvedType>(), [], []);
            ...
            const errconskey = this.m_emitter.registerStaticCall(ootype, errconsfunc, "create", new Map<string, ResolvedType>(), [], []);
    ...
    private checkNamespaceOperatorInvoke(sinfo: SourceInfo, env: TypeEnvironment, opdecl: NamespaceOperatorDecl, args: MIRArgument[], argtypes: ValueType[], refs: ["ref" | "out" | "out?", MIRParameterVariable | MIRLocalVariable, ResolvedType][], pcodes: PCode[], cinfo: [string, ResolvedType][], recursive: RecursiveAnnotation, trgt: MIRTempRegister, refok: boolean): TypeEnvironment[] {
    ...
    private checkStaticOperatorInvoke(sinfo: SourceInfo, env: TypeEnvironment, oodecl: OOPTypeDecl, oobinds: Map<string, ResolvedType>, opdecl: StaticOperatorDecl, args: MIRArgument[], argtypes: ValueType[], refs: ["ref" | "out" | "out?", MIRParameterVariable | MIRLocalVariable, ResolvedType][], pcodes: PCode[], cinfo: [string, ResolvedType][], recursive: RecursiveAnnotation, trgt: MIRTempRegister, refok: boolean): TypeEnvironment[] {
        const ootype = this.m_emitter.registerResolvedTypeReference(this.resolveOOTypeFromDecls(oodecl, oobinds));
        ...
        if (!opdecl.isDynamic || !OOPTypeDecl.attributeSetContains("abstract", opdecl.attributes)) {
            ...
            const opkey = this.m_emitter.registerStaticOperatorCall(ootype, opdecl.name, opdecl, sigkeys, oobinds, pcodes, cinfo);
            const refinfo = this.generateRefInfoForCallEmit(fsig as ResolvedFunctionType, refs);
            this.m_emitter.emitInvokeFixedFunction(sinfo, opkey, cargs, "[IGNORE]", refinfo, trgt);
        }
        else {
            ...
            const opkey = this.m_emitter.registerVirtualStaticOperatorCall(ootype, opdecl.name, opdecl, oobinds, pcodes, cinfo);
            const refinfo = this.generateRefInfoForCallEmit(fsig as ResolvedFunctionType, refs);
            this.m_emitter.emitInvokeVirtualOperator(sinfo, opkey, cargs,  refinfo, trgt);
        }
    ...
    private checkCallStaticFunctionOrOperatorExpression(env: TypeEnvironment, exp: CallStaticFunctionOrOperatorExpression, trgt: MIRTempRegister, refok: boolean): TypeEnvironment[] {
        ...
        if(fdecltry !== undefined && fdecltry.contiainingType.ns === "NSCore" && fdecltry.contiainingType.name === "KeyType" && (exp.name === "less" || exp.name === "equal")) {
            ...
            if (opdecltry !== undefined) {
                ...
                const ckey = this.m_emitter.registerStaticCall(ootype, fdecl.decl, exp.name, callbinds, rargs.pcodes, rargs.cinfo);
                const refinfo = this.generateRefInfoForCallEmit(fsig as ResolvedFunctionType, rargs.refs);
                this.m_emitter.emitInvokeFixedFunction(exp.sinfo, ckey, rargs.args, rargs.fflag, refinfo, trgt);
    ...
    private checkModifyWithIndecies(env: TypeEnvironment, op: PostfixModifyWithIndecies, arg: MIRTempRegister, trgt: MIRTempRegister): TypeEnvironment {
        ...
        if (texp.flowtype.isUniqueTupleTargetType()) {
            ...
            const invk = this.m_emitter.registerTupleUpdateVirtual(texp, updates);
    ...
    private checkModifyWithNames(env: TypeEnvironment, op: PostfixModifyWithNames, arg: MIRTempRegister, trgt: MIRTempRegister): TypeEnvironment {
        ...
        if (texp.flowtype.isRecordTargetType()) {
            ...
            if (texp.flowtype.isUniqueRecordTargetType()) {
                ...
                const invk = this.m_emitter.registerRecordUpdateVirtual(texp, updates);
                this.m_emitter.emitInvokeVirtualFunction(op.sinfo, invk, this.m_emitter.registerResolvedTypeReference(texp.layout), this.m_emitter.registerResolvedTypeReference(texp.flowtype), [arg, ...updates.map((upd) => upd[2])], undefined, this.m_emitter.registerResolvedTypeReference(texp.flowtype), trgt);
            }
    ...
    private checkPostfixGetIndexOrNone(env: TypeEnvironment, op: PostfixGetIndexOrNone, arg: MIRTempRegister, trgt: MIRTempRegister): TypeEnvironment {
        ...
        if(hpi === "no") {
            ...
            const guard = new MIRVarGuard(hasreg);
            ...
            if(ttype.isSameType(linfo)) {
                this.m_emitter.emitRegisterStore(op.sinfo, loadreg, trgt, this.m_emitter.registerResolvedTypeReference(linfo), guard);
            }
            else {
                this.m_emitter.emitConvert(op.sinfo, this.m_emitter.registerResolvedTypeReference(ttype), this.m_emitter.registerResolvedTypeReference(ttype), this.m_emitter.registerResolvedTypeReference(linfo), loadreg, trgt, guard);
            }
    ...
    private checkPostfixGetIndexTryMulti(env: TypeEnvironment, op: PostfixGetIndexTry, arg: MIRTempRegister, trgt: MIRTempRegister): TypeEnvironment[] {
        ...
        if (hpi === "no") {
            ...
            this.m_emitter.emitLoadTupleIndexSetGuard(op.sinfo, arg, this.m_emitter.registerResolvedTypeReference(texp.layout), this.m_emitter.registerResolvedTypeReference(texp.flowtype), op.idx, !texp.flowtype.isUniqueTupleTargetType(), this.m_emitter.registerResolvedTypeReference(linfo), env.getLocalVarInfo(op.vname) !== undefined ? new MIRLocalVariable(op.vname) : new MIRParameterVariable(op.vname), new MIRVarGuard(trgt));
            return [
    ...
    private checkPostfixGetPropertyOrNone(env: TypeEnvironment, op: PostfixGetPropertyOrNone, arg: MIRTempRegister, trgt: MIRTempRegister): TypeEnvironment {
        ...
        if(hpi === "no") {
            ...
            if(rtype.isSameType(linfo)) {
                ...
                this.m_emitter.emitConvert(op.sinfo, this.m_emitter.registerResolvedTypeReference(rtype), this.m_emitter.registerResolvedTypeReference(rtype), this.m_emitter.registerResolvedTypeReference(linfo), loadreg, trgt, guard);
            }
    ...
    private checkPostfixGetPropertyTryMulti(env: TypeEnvironment, op: PostfixGetPropertyTry, arg: MIRTempRegister, trgt: MIRTempRegister): TypeEnvironment[] {
        ...
        if(hpi === "no") {
            ...
            this.m_emitter.emitLoadRecordPropertySetGuard(op.sinfo, arg, this.m_emitter.registerResolvedTypeReference(texp.layout), this.m_emitter.registerResolvedTypeReference(texp.flowtype), op.pname, !texp.flowtype.isUniqueTupleTargetType(), this.m_emitter.registerResolvedTypeReference(linfo), env.getLocalVarInfo(op.vname) !== undefined ? new MIRLocalVariable(op.vname) : new MIRParameterVariable(op.vname), new MIRVarGuard(trgt));
            return [
                env.setBoolResultExpression(this.m_assembly.getSpecialBoolType(), FlowTypeTruthValue.False),
                env.setVar(op.vname, linfo).setBoolResultExpression(this.m_assembly.getSpecialBoolType(), FlowTypeTruthValue.True)
            ];
    ...
    private checkInvokeMulti(env: TypeEnvironment, op: PostfixInvoke, arg: MIRTempRegister, trgt: MIRTempRegister, refok: boolean): TypeEnvironment[] {
        ...
        if (knownimpl !== undefined) {
            ...
            const ckey = this.m_emitter.registerMethodCall(ootype, knownimpl.decl, op.name, callbinds, rargs.pcodes, rargs.cinfo);
            const refinfo = this.generateRefInfoForCallEmit(fsig as ResolvedFunctionType, rargs.refs);
            ...
            const ootype = this.m_emitter.registerResolvedTypeReference(this.resolveOOTypeFromDecls(minfo.contiainingType, minfo.binds));
    ...
    private checkBinEq(env: TypeEnvironment, exp: BinEqExpression, trgt: MIRTempRegister): TypeEnvironment[] {
        ...
        if(action.chk === "truealways" || action.chk === "falsealways") {
            ...
            if (action.chk === "stringof" || action.chk === "datastring") {
                ...
                const ikey = this.m_emitter.registerMethodCall(this.m_emitter.registerResolvedTypeReference(olhs.flowtype), sobj.object.memberMethods.get("string") as MemberMethodDecl, "string", new Map<string, ResolvedType>(), [], []);
    ...
    private checkDeclareSingleVariable(sinfo: SourceInfo, env: TypeEnvironment, vname: string, isConst: boolean, decltype: TypeSignature, venv: { etype: ValueType, etreg: MIRTempRegister } | undefined): TypeEnvironment {
        ...
        if (venv !== undefined) {
            ...
            this.m_emitter.emitRegisterStore(sinfo, convreg, new MIRLocalVariable(vname), mirvtype, undefined);
        }
    ...
    private checkAssignSingleVariable(sinfo: SourceInfo, env: TypeEnvironment, vname: string, etype: ValueType, etreg: MIRTempRegister): TypeEnvironment {
        ...
        if (env.getLocalVarInfo(vname) !== undefined) {
            this.m_emitter.emitRegisterStore(sinfo, convreg, new MIRLocalVariable(vname), this.m_emitter.registerResolvedTypeReference((vinfo as VarInfo).declaredType), undefined);
        }
        else {
            this.m_emitter.emitRegisterStore(sinfo, convreg, new MIRParameterVariable(vname), this.m_emitter.registerResolvedTypeReference((vinfo as VarInfo).declaredType), undefined);
        }
    ...
    private generateRootRegOptional(sinfo: SourceInfo, path: StructuredAssignmentPathStep, ereg: MIRRegisterArgument): [MIRTempRegister, MIRTempRegister, ResolvedType] {
            ...
            if (path.step === "tuple") {
                this.m_emitter.emitLoadTupleIndexSetGuard(sinfo, ereg, fromtype, fromtype, path.ival, path.fromtype.isUniqueTupleTargetType(), loadtype, nreg, new MIRVarGuard(greg));
            }
            else {
                this.m_emitter.emitLoadRecordPropertySetGuard(sinfo, ereg, fromtype, fromtype, path.nval, path.fromtype.isUniqueRecordTargetType(), loadtype, nreg, new MIRVarGuard(greg));
            }
        ...
        if (okenvs.length === 0) {
            ...
            for (let i = 0; i < elemtc.length; ++i) {
                ...
                if (tcc[1].isoptional) {
                    ...
                    results.push(ncr);
                    this.m_emitter.emitTypeOf(sinfo, ncr, this.m_emitter.registerResolvedTypeReference(tcc[1].oftype as ResolvedType), treg, this.m_emitter.registerResolvedTypeReference(ttype), this.m_emitter.registerResolvedTypeReference(ttype), new MIRVarGuard(greg));
                }
                else {
    ...
    private checkValidateStatement(env: TypeEnvironment, stmt: ValidateStatement): TypeEnvironment {
        ...
        if(flow.fenvs.length === 0) {
            ...
            if (stmt.err instanceof LiteralNoneExpression) {
                ...
                const errconsfunc = errtdcl.staticFunctions.get("create") as StaticFunctionDecl;
                const errconskey = this.m_emitter.registerStaticCall(mirerrconstype, errconsfunc, "create", new Map<string, ResolvedType>(), [], []);
    ...
    private emitPrelogForRefParamsAndPostCond(sinfo: SourceInfo, env: TypeEnvironment, refparams: string[]): string[] {
        ...
        for(let i = 0; i < refparams.length; ++i) {
            ...
            this.m_emitter.emitRegisterStore(sinfo, new MIRParameterVariable(refparams[i]), new MIRLocalVariable(`$${refparams[i]}`), this.m_emitter.registerResolvedTypeReference(rpt), undefined);
        }
    ...
    private emitPrologForReturn(sinfo: SourceInfo, refparams: string[], rrinfo: { declresult: MIRType, runtimeresult: MIRType, elrcount: number, refargs: [string, MIRType][] }, wpost: boolean): MIRArgument[] {
        if(wpost) {
            this.m_emitter.emitRegisterStore(sinfo, new MIRLocalVariable("$__ir_ret__"), new MIRLocalVariable("$$__post_arg__"), rrinfo.declresult, undefined);
        }

        let rargs: MIRArgument[] = [new MIRLocalVariable("$$__post_arg__")];
        if(rrinfo.refargs.length === 0) {
            this.m_emitter.emitRegisterStore(sinfo, new MIRLocalVariable("$__ir_ret__"), new MIRLocalVariable("$$return"), rrinfo.declresult, undefined);
        }
        else {
    ...
    private checkBodyStatement(srcFile: string, env: TypeEnvironment, body: BlockStatement, optparaminfo: { pname: string, ptype: ResolvedType, maskidx: number, initaction: InitializerEvaluationAction }[], outparaminfo: { pname: string, defonentry: boolean, ptype: ResolvedType }[], args: Map<string, MIRType>, preject: [{ ikey: string, sinfo: SourceInfo, srcFile: string }[], string[]] | undefined, postject: [{ ikey: string, sinfo: SourceInfo, srcFile: string }[], string[]] | undefined | undefined): MIRBody | undefined {
        ...
        for (let i = 0; i < optparaminfo.length; ++i) {
            ...
            const guard = new MIRMaskGuard("#maskparam#", opidx);
            if(iv instanceof InitializerEvaluationLiteralExpression) {
                ...
                this.m_emitter.emitRegisterStore(body.sinfo, this.emitInlineConvertIfNeeded(iv.constexp.sinfo, ttmp, oftt, oftype), storevar, this.m_emitter.registerResolvedTypeReference(oftype), guard);
            }
            else if (iv instanceof InitializerEvaluationConstantLoad) {
                this.m_emitter.emitRegisterStore(body.sinfo, new MIRGlobalVariable(iv.gkey), storevar, this.m_emitter.registerResolvedTypeReference(oftype), guard);
            }
            else {
                const civ = iv as InitializerEvaluationCallAction;
                this.m_emitter.emitInvokeFixedFunctionWithGuard(body.sinfo, civ.ikey, civ.args, undefined, this.m_emitter.registerResolvedTypeReference(oftype), storevar, guard);
            }
    ...
    private processExpressionForGobalConstExpr(ikey: MIRInvokeKey, decl: NamespaceConstDecl): MIRGlobalKey {
        ...
        try {
            ...
            const mirglobal = new MIRConstantDecl(undefined, iname, ikey, decl.sourceLocation, decl.srcFile, dtype.trkey, gkey);
    ...
    private processExpressionForStaticConstExpr(ikey: MIRInvokeKey, containingType: OOPTypeDecl, decl: StaticMemberDecl, binds: Map<string, ResolvedType>): MIRGlobalKey {
        ...
        try {
            ...
            const dtype = this.m_emitter.registerResolvedTypeReference(ddecltype);
            const mirconst = new MIRConstantDecl(enclosingType, iname, ikey, decl.sourceLocation, decl.srcFile, dtype.trkey, gkey);
    ...
    private generateSpecialVerificationAssert(chkkey: MIRInvokeKey, tdecl: EntityTypeDecl, binds: Map<string, ResolvedType>, clauses: { ikey: string, sinfo: SourceInfo, srcFile: string, args: string[] }[]): boolean {
        ...
        allfields.forEach((ff) => {
            ...
            this.m_emitter.emitRegisterStore(ff[1].sourceLocation, lreg, new MIRLocalVariable(`$${ff[1].name}`), this.m_emitter.registerResolvedTypeReference(ftype), undefined);
        });
    ...
    private generateConstructor(env: TypeEnvironment, conskey: MIRInvokeKey, tdecl: EntityTypeDecl, binds: Map<string, ResolvedType>): { ikey: string, sinfo: SourceInfo, srcFile: string, args: string[] }[] {
        ...
        for(let i = 0; i < optfields.length; ++i) {
            ...
            if(iv instanceof InitializerEvaluationLiteralExpression) {
                ...
                this.m_emitter.emitRegisterStore(ofi[1].sourceLocation, this.emitInlineConvertIfNeeded(ofi[1].sourceLocation, ttmp, oftt, oftype), storevar, this.m_emitter.registerResolvedTypeReference(oftype), guard);
            }
            else if (iv instanceof InitializerEvaluationConstantLoad) {
                this.m_emitter.emitRegisterStore(ofi[1].sourceLocation, new MIRGlobalVariable(iv.gkey), storevar, this.m_emitter.registerResolvedTypeReference(oftype), guard);
            }
            else {
                const civ = iv as InitializerEvaluationCallAction;
                this.m_emitter.emitInvokeFixedFunctionWithGuard(ofi[1].sourceLocation, civ.ikey, civ.args, undefined, this.m_emitter.registerResolvedTypeReference(oftype), storevar, guard);
            }
    ...
    private processInvokeInfo(fname: string, enclosingDecl: [MIRType, OOPTypeDecl, Map<string, ResolvedType>] | undefined, kind: "namespace" | "static" | "member", iname: string, ikey: MIRInvokeKey, sinfo: SourceInfo, invoke: InvokeDecl, binds: Map<string, ResolvedType>, pcodes: PCode[], pargs: [string, ResolvedType][]): MIRInvokeDecl {
        ...
        const resultType = this.generateExpandedReturnSig(sinfo, declaredResult, entrycallparams);
        ...
        if(kind === "namespace") {
            ...
            if (invoke.preconditions.every((pre) => pre.isvalidate)) {
                realbody = this.processGenerateSpecialPreFunction_ResultT(sinfo, invoke.preconditions, invoke.body as BodyImplementation);
            }
            else {
        ...
        if (typeof ((invoke.body as BodyImplementation).body) === "string") {
            ...
            return new MIRInvokePrimitiveDecl(encdecl, fname, iname, ikey, invoke.attributes, recursive, sinfo, invoke.srcFile, mbinds, params, resultType.trkey, (invoke.body as BodyImplementation).body as string, mpc);
        }
        else {
