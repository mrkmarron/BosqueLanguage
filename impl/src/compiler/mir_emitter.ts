...
class MIREmitter {
    ...
    emitTupleHasProperty(sinfo: SourceInfo, arg: MIRArgument, argtype: MIRType, idx: number, isvirtual: boolean, hasflag: string, position: number) {
        if(!this.emitEnabled) {
            return;
        }
    ...
    emitRecordHasProperty(sinfo: SourceInfo, arg: MIRArgument, argtype: MIRType, pname: string, isvirtual: boolean, hasflag: string, position: number) {
        if(!this.emitEnabled) {
            return;
        }
    ...
    emitInvokeFixedFunction(sinfo: SourceInfo, ikey: MIRInvokeKey, args: MIRArgument[], optstatusmask: string | undefined, rretinfo: MIRType | [MIRType, MIRType, number, [string, MIRType][]], trgt: MIRTempRegister) {
        if(!this.emitEnabled) {
            return;
        }

        const retinfo = Array.isArray(rretinfo) ? rretinfo : [rretinfo, rretinfo, -1, []] as [MIRType, MIRType, number, [string, MIRType][]];
        ...
        if (retinfo[3].length === 0) {
            ...
            for (let i = 0; i < retinfo[3].length; ++i) {
                ...
                this.m_currentBlock.push(new MIRParameterVarStore(sinfo, tr, new MIRParameterVariable(retinfo[3][i][0])));
            }
        }
    }
    ...
    */
    ...
    registerOperatorCall(ns: string, name: string, opdecl: NamespaceOperatorDecl, pcodes: PCode[], cinfo: [string, ResolvedType][]): MIRInvokeKey {
        xxxx;
    }
