...
class MIREmitter {
    ...
    emitConstructorPrimaryCollectionSingletons(sinfo: SourceInfo, tkey: MIRResolvedTypeKey, args: MIRArgument[], trgt: MIRRegisterArgument) {
        if(!this.emitEnabled) {
            return;
        }
    ...
    emitConstructorPrimaryCollectionCopies(sinfo: SourceInfo, tkey: MIRResolvedTypeKey, args: MIRArgument[], trgt: MIRRegisterArgument) {
        if(!this.emitEnabled) {
            return;
        }
    ...
    emitConstructorPrimaryCollectionMixed(sinfo: SourceInfo, tkey: MIRResolvedTypeKey, args: [boolean, MIRArgument][], trgt: MIRRegisterArgument) {
        if(!this.emitEnabled) {
            return;
        }
    ...
    emitNoneJump(sinfo: SourceInfo, arg: MIRRegisterArgument, noneblck: string, someblk: string, ) {
        if(!this.emitEnabled) {
            return;
        }
    ...
    getVCallInstantiations(assembly: Assembly): [MIRVirtualMethodKey, MIRInvokeKey, string, string, [MIRType, OOPTypeDecl, Map<string, ResolvedType>], MemberMethodDecl, Map<string, ResolvedType>, PCode[], [string, ResolvedType][]][] {
        ...
        for (let i = 0; i < this.allVInvokes.length; ++i) {
            ...
            for (let j = 0; j < impls.length; ++j) {
                ...
                const mcreate = assembly.tryGetMethodUniqueConcreteDeclFromType(itype, vinv[3]);
        ...
        if(optns !== undefined) {
            ...
            ootype.staticOperators.forEach((op) => {
                if (op[1].name === name && !OOPTypeDecl.attributeSetContains("abstract", op[1].invoke.attributes)) {
                    ...
                    impls.push([vkey, okey, op[1].name, `${enclosingType[0].trkey}::${op[1].name}`, enclosingType, op[1].invoke, binds, pcodes, cargs]);
                }
            });
        }
