...
Commander
    ...
    .option("-c --compiler [compiler]", "Compiler to use", "clang")
...
setImmediate(() => {
    console.log(`Transpiling Bosque assembly to C++...`);
    ...
    console.log(`Compiling C++ code with ${Commander.compiler} into exe file "${Commander.outfile}"...`);
    process.chdir(cpppath);
    execSync(`${Commander.compiler}${Commander.debug ? " -g -DBDEBUG " : ""}-o ${Commander.outfile} *.cpp`);
