...
public:
    ...
    static BSQList<T, RCDecF, DisplayF>* set_entry_list(Ty* s)
    {
        ...
        std::transform(s->entries.begin(), s->entries.end(), std::back_inserter(entries), [](T& v) -> v {
            return RCIncF{}(v);
        });
    ...
    static bool set_subsetof(Ty* s, Ty* os)
    {
        ...
        return std::all_of(s->entries.begin(), s->entries.end(), [s](const T& v) -> bool {
    ...
    static bool set_disjoint(Ty* s, Ty* os)
    {
        ...
        return std::none_of(s->entries.begin(), s->entries.end(), [s](const T& v) -> bool {
            return std::binary_search(os->entries.begin(), os->entries.end(), v, T_CMP{});  
        });
    }
    ...
    {
        std::vector<T> entries;
        std::for_each(s->entries.begin(), s->entries.end(), [&entries](const T& v) -> void {
            if(LambdaP{}(v))
            {
    ...
    static BSQSet<U, U_RCDecF, U_DisplayF, U_CMP, U_EQ>* set_oftype(Ty* std)
    {
    ...
    static Ty* set_union(Ty* s, Ty* os)
    {
        ...
        std::for_each(s->entries.begin(), s->entries.end(), [](T& v) -> v {
            return RCIncF{}(v);
        });
    ...
    static Ty* set_unionall(ListT* sl)
    {
        ...
        std::for_each(sl->entries.begin() + 1, sl->entries.end(), [&sentries](Ty* s) -> void {
        ...
        std::transform(sentries.begin(), sentries.end(), std::back_inserter(entries), [](T& v) -> v {
            return RCIncF{}(v);
        });
    ...
    template <typename LL_RCDecF, typename LL_DisplayF>
